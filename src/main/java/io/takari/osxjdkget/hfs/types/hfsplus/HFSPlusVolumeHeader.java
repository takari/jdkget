...
import io.takari.osxjdkget.csjc.MutableStruct;
import io.takari.osxjdkget.csjc.PrintableStruct;
import io.takari.osxjdkget.csjc.StructElements;
import io.takari.osxjdkget.csjc.structelements.ASCIIStringField;
...
import io.takari.osxjdkget.hfs.types.hfs.HFSVolumeFinderInfo;
...
public class HFSPlusVolumeHeader extends MutableStruct implements
  ...
  private HFSPlusVolumeHeader(boolean mutable, byte[] data, int offset) {
    ...
    nextCatalogID = (mutable ? new HFSCatalogNodeID.Mutable(data, offset + 64) : new HFSCatalogNodeID(data, offset + 64));
    System.arraycopy(data, offset + 68, writeCount, 0, 4);
    System.arraycopy(data, offset + 72, encodingsBitmap, 0, 4);
    finderInfo = (mutable ? new HFSVolumeFinderInfo.Mutable(data, offset + 80) : new HFSVolumeFinderInfo(data, offset + 80));
    //System.arraycopy(data, 112, allocationFile, 0, 80);
    allocationFile = (mutable ? new HFSPlusForkData.Mutable(data, offset + 112) : new HFSPlusForkData(data, offset + 112));
    //System.arraycopy(data, 192, extentsFile, 0, 80);
    extentsFile = (mutable ? new HFSPlusForkData.Mutable(data, offset + 192) : new HFSPlusForkData(data, offset + 192));
    //System.arraycopy(data, 272, catalogFile, 0, 80);
    catalogFile = (mutable ? new HFSPlusForkData.Mutable(data, offset + 272) : new HFSPlusForkData(data, offset + 272));
    //System.arraycopy(data, 352, attributesFile, 0, 80);
    attributesFile = (mutable ? new HFSPlusForkData.Mutable(data, offset + 352) : new HFSPlusForkData(data, offset + 352));
    //System.arraycopy(data, 432, startupFile, 0, 80);
    startupFile = (mutable ? new HFSPlusForkData.Mutable(data, offset + 432) : new HFSPlusForkData(data, offset + 432));
  }
