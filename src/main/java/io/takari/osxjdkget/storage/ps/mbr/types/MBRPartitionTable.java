...
package io.takari.osxjdkget.storage.ps.mbr.types;
...
import io.takari.osxjdkget.storage.ps.ebr.types.DOSExtendedPartitionSystem;
import io.takari.osxjdkget.storage.ps.legacy.PartitionSystem;
import io.takari.osxjdkget.storage.ps.mbr.MBRPartitionType;
...
public class MBRPartitionTable implements PartitionSystem, PrintableStruct {
  ...
  public MBRPartitionTable(ReadableRandomAccessStream raf, int offset, int sectorSize, boolean parseEmbeddedPartitionSystems) {
    ...
    for (int i = 0; i < mbrPartitions.length; ++i) {
      ...
      if (!parseEmbeddedPartitionSystems)
        ; // Disable all other elses
