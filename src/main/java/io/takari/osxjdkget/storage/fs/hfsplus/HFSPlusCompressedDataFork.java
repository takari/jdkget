...
package io.takari.osxjdkget.storage.fs.hfsplus;
...
import io.takari.osxjdkget.hfs.types.decmpfs.DecmpfsHeader;
import io.takari.osxjdkget.hfsexplorer.fs.ResourceForkReader;
import io.takari.osxjdkget.hfsexplorer.types.resff.ReferenceListEntry;
import io.takari.osxjdkget.hfsexplorer.types.resff.ResourceMap;
import io.takari.osxjdkget.hfsexplorer.types.resff.ResourceType;
import io.takari.osxjdkget.io.BasicReadableRandomAccessStream;
      ...
      if (compressionType == DecmpfsHeader.COMPRESSION_TYPE_INLINE) {
        ...
        if ((compressionFlags & 0x0F) == 0x0F) {
          ...
          final byte[] uncompressedData =
            io.takari.osxjdkget.util.IOUtil.readFully(decmpfsForkStream,
  ...
  private static class CompressedResourceStream
    ...
    public synchronized int read(byte[] data, int pos, int len)
      ...
      while (curFp < endFp) {
        ...
        if (!skip) {
          ...
          if ((compressionFlags & 0x0F) == 0x0F) {
            ...
            final int curBytesToRead =
              remainingInBlock < remainingLen ? remainingInBlock : remainingLen;
            ...
            while (!inflater.finished()) {
              ...
              if (curOffsetInBlock >= curDecompressedOffsetInBlock &&
                curOffsetInBlock < (curDecompressedOffsetInBlock +
                  inflatedBytes)) {
                final int inOffset =
                ...
                final int copyLength =
                  remainingBytes < inflatedBytes ? remainingBytes : inflatedBytes;
          ...
          if (curBlock == processedBlocks) {
            ...
            if (processedBlocks == 0) {
              ...
              if (nextBlockOffsets == null ||
                nextBlockOffsets.length < (processedBlocks + 1)) {
        ...
        curBlockOffset += fixedBlockSize != 0 ? fixedBlockSize : nextBlockOffsets[curBlock] -
